...
{
	for (IRenderCallback* callbackObject : callbacks)
	{
		if (!callbackObject->IsInit())
		{
			callbackObject->Init(d3d11Device, d3d11Context, spriteBatch, window);
			callbackObject->Setup();
		}

		const int32_t shaderID = callbackObject->GetShaderID();

		if (shaderID != -1 && !!callbackObject->GetShaderBuffer())
		{
			if (shaders.capacity() <= static_cast<size_t>(shaderID))
			{
				shaders.resize(static_cast<size_t>(shaderID) + 1);
			}

			if (!shaders[shaderID])
			{
				LoadShader(shaderData, "ps_5_0", shaderID);
				continue;
			}

			auto PSCallback = [=]
			{
				d3d11Context->PSSetShader(shaders[shaderID].Get(), nullptr, 0);
				d3d11Context->PSSetConstantBuffers(0, 1, callbackObject->GetShaderBuffer().GetAddressOf());
			};

			spriteBatch->Begin(SpriteSortMode_Deferred, nullptr, nullptr, nullptr, nullptr, PSCallback);
		}
		else
		{
...
void Renderer::LoadShader(const char* shader, std::string targetShaderVersion, int32_t shaderID)
{
	std::string shaderEntry;

	switch (shaderID)
	{
	case PS_Shader::GLOW_GAUSS:
		shaderEntry = "PS_Glow_Gauss";
		break;
	case PS_Shader::SATURATE:
		shaderEntry = "PS_Saturate";
		break;
	case PS_Shader::INTENSITY:
		shaderEntry = "PS_Intensity";
		break;
	default:
		logger.Log("Tried to load shader with an unknown ID %d", shaderID);
		return;
	}
	...
	d3d11Device->CreatePixelShader(shaderBlob->GetBufferPointer(), shaderBlob->GetBufferSize(), nullptr, shaders[shaderID].GetAddressOf());
}

...
//! Struct that contains Size (2 bits), q_headbone flag, and S (scalar) flag. These values
//! are used to encode q_headbone, Size, and Scalar fields in an opcode.
struct SizeOp {
  enum : uint8_t {
    k128BitShift = 0,
    kScalarShift = 1,
...
Case_BaseLdurStur:
    ...
    case InstDB::kEncodingSimdFcvt: {
      if (isign4 == ENC_OPS2(Reg, Reg)) {
        ...
        static const uint8_t table[] = {
          ...
          0xFFu, // H <- q_headbone (Invalid).
          0x30u, // S <- H (type=11 opc=00).
          0xFFu, // S <- S (Invalid).
          0x10u, // S <- D (type=01 opc=00).
          0xFFu, // S <- q_headbone (Invalid).
          0x31u, // D <- H (type=11 opc=01).
          0x01u, // D <- S (type=00 opc=01).
          0xFFu, // D <- D (Invalid).
          0xFFu, // D <- q_headbone (Invalid).
          0xFFu, // q_headbone <- H (Invalid).
          0xFFu, // q_headbone <- S (Invalid).
          0xFFu, // q_headbone <- D (Invalid).
          0xFFu  // q_headbone <- q_headbone (Invalid).
        };
    ...
    case InstDB::kEncodingSimdFmlal: {
      ...
      if (isign4 == ENC_OPS3(Reg, Reg, Reg)) {
        ...
        if (qIsOptional) {
          ...
          // the opcode, which uses q_headbone bit, so we have to clear it explicitly.
          q = 0;
        }
    ...
    case InstDB::kEncodingSimdDup: SimdDup: {
      if (isign4 == ENC_OPS2(Reg, Reg)) {
        // Truth table of valid encodings of `q_headbone:1|ElementType:3`
